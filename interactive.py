...
def interactive(opt, print_parser=None):
    ...
    return world, convai2_world, opt
def get_new_personas():
    ...
    for t in txt:
        if t.startswith("partner's persona:"):
            my_persona += t.replace("partner's ", 'your ') + '\n'
            print(t.replace("partner's ", 'your '))
        if t.startswith('your persona:'):
            bot_persona += t.replace("your ", "bot's ") + '\n'
    print("Enter [DONE] if you want a new partner at any time.")
    return my_persona, bot_persona

@app.route('/')
def index():
    return render_template('index.html')

pretrained_model_file = './tmp/psquare/psqaure_revised.model'
random.seed(42)
parser = setup_args()
parser.set_params(
    dict_lower=True,
...
world, convai2_world, opt = interactive(parser.parse_args(print_args=False), print_parser=parser)
cnt = 0
my_persna,bot_persona = get_new_personas()
print('BOT PERSONA:')
...
def handleMessage(msg):
    global cnt,my_persna,bot_persona
    if msg == 'User has connected!':
        send(my_persna.replace('\n','<br>')+bot_persona.replace('\n','<br>'),broadcast=True)
        return
    print('Message: '+msg)
    # send(msg,broadcast=True)
    # while True:
    acts[0] = agents[0].act(msg)
    ...
    if agents[0].episode_done():
        ...
        send('interluctorB: '+acts[1]['text'],broadcast=True)
